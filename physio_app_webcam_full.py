#!/usr/bin/env python3
"""
physio_app_webcam_full.py — GaitLab Pro  (webcam build)
ALL LOGIC PRESERVED EXACTLY. Visual layer completely rebuilt — dark dashboard.
"""
...
from PySide6.QtCore import Qt, QThread, Signal, Slot, QTimer, QMargins, QRect
from PySide6.QtGui  import (QImage, QPixmap, QFont, QColor, QBrush,
                             QLinearGradient, QPainter, QPen, QGradient)
...
#
#  Page:    warm near-black        #0C0D0E
#  Cards:   dark charcoal          #161819  /  #1E2224
#  Orange:  primary accent         #F97316
#  Yellow:  secondary accent       #FCD34D
#  Green:   success / locked       #22C55E
#  Magenta: tertiary               #E879F9
#  Rose:    danger / REC           #F43F5E
#
# Angle card accents (one per joint):
#   L knee  →  orange  #F97316
#   R knee  →  yellow  #FCD34D
#   L hip   →  green   #22C55E
#   R hip   →  magenta #E879F9

BG   = "#0C0D0E"    # page background — warm near-black
...
# Per-joint accent colours — orange, yellow, green, magenta
...
# ═══════════════════════════════════════════════════════════════════════════════
#  GAIT COMPUTATION  (preserved exactly)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  POSE HELPERS  (preserved)
# ═══════════════════════════════════════════════════════════════════════════════
def angle_world(world, a, b, c):
    pa, pb, pc = world[IDX[a]], world[IDX[b]], world[IDX[c]]
...
def person_size(landmarks, w, h):
    sh  = landmarks[IDX["LEFT_SHOULDER"]], landmarks[IDX["RIGHT_SHOULDER"]]
    hp  = landmarks[IDX["LEFT_HIP"]],      landmarks[IDX["RIGHT_HIP"]]
    shy = (sh[0].y+sh[1].y)/2*h
    hpy = (hp[0].y+hp[1].y)/2*h
    return abs(hpy - shy)


# ═══════════════════════════════════════════════════════════════════════════════
#  POSE ENGINE  (preserved)
# ═══════════════════════════════════════════════════════════════════════════════
class PoseEngine(QThread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            people, worlds = res.pose_landmarks, res.pose_world_landmarks
            ...
            if people:
                ...
                sizes   = [person_size(p, w, h) for p in people]
                ...
                if self.patient_center is None:
                    ...
                    for i, (c, s) in enumerate(zip(centers, sizes)):
                        pos_cost  = np.linalg.norm(c - self.patient_center) / max(w*0.25, 1)
                        size_cost = abs(s - self.patient_size) / max(self.patient_size, 1)
                        cost      = pos_cost + size_cost * 2.5
                        if cost < best_cost:
                ...
                if pi is not None:
                    ...
                    if self.patient_size is None:
                        self.patient_size = sizes[pi]
                    else:
                        self.patient_size = 0.85*self.patient_size + 0.15*sizes[pi]
                    self._locked = True
                else:
                    self._locked = False
            else:
            ...
            if people:
                for k, p in enumerate(people):
                    ...
                    if k == pi:
                        for jt in p:
                            cv2.circle(overlay, (int(jt.x*w), int(jt.y*h)),
                                       5, (30, 140, 255), -1)
            ...
            if pi is not None:
                ...
                for k in range(len(NAMES)):
                    ...
                    visd[NAMES[k]] = float(vv) if vv is not None else 1.0

            def joint_ok(name):
                return pi is not None and visd.get(name, 0.0) >= VIS_THRESHOLD

            smoothed = {}
            for lab, (a, b, c) in ANGLES.items():
                if joint_ok(a) and joint_ok(b) and joint_ok(c):
                    raw = angle_world(worlds[pi], a, b, c)
            ...
            if pi is not None:
                wl = worlds[pi]
                p3 = {NAMES[k]: (wl[k].x, wl[k].y, wl[k].z) for k in range(len(NAMES))}
...
class AngleCard(QFrame):
    """Dark card with coloured accent stripe + huge bright number."""
    def __init__(self, label, parent=None):
        ...
        self.setStyleSheet(
            f"QFrame#AC{{background:{C1}; border-radius:14px; "
            f"border:1px solid {C3};}}")
        ...
        self._lbl.setStyleSheet(
            f"color:{T2}; font-size:10px; font-weight:700; "
        ...
        self._deg.setStyleSheet(
            f"color:{self._accent}; font-size:18px; opacity:0.7; "
            f"border:none; background:transparent;")

        iv.addWidget(self._lbl)
        row = QHBoxLayout(); row.setSpacing(2)
        row.addWidget(self._val)
        row.addWidget(self._deg, 0, Qt.AlignBottom)
        row.addStretch()
        iv.addLayout(row)
...
class MetricCard(QFrame):
    """Dark metric card with coloured value."""
    def __init__(self, label, unit="", na="—", large=True,
                 accent=CY, parent=None):
        super().__init__(parent)
        self._na     = na
        self._large  = large
        self._accent = accent
        ...
        self.setStyleSheet(
            f"QFrame#MCd{{background:{C1}; border-radius:12px; "
            f"border:1px solid {C3};}}")
        ...
        row.addWidget(self._val)
        if unit:
            row.addWidget(self._unit_lbl, 0, Qt.AlignBottom)
        row.addStretch()
        v.addLayout(row)
...
class TopBar(QWidget):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(f"QWidget#TB{{background:{TB1}; "
                           f"border-bottom:1px solid {C3};}}")
        ...
        tag  = QLabel("Clinical Motion Analysis")
        tag.setStyleSheet(f"color:{T2}; font-size:10px; border:none;")
...
def _fl(text):
    l = QLabel(text)
    l.setStyleSheet(
        f"color:{T2}; font-size:11px; font-weight:600; "
...
class SetupScreen(QWidget):
    def __init__(self, main):
        ...
        # ── left: dark form card ──────────────────────────────────────────
        left = QWidget()
        left.setFixedWidth(376)
        left.setObjectName("LP")
        left.setStyleSheet(
            f"QWidget#LP{{background:{C1}; border-right:1px solid {C3};}}")
        ...
        title.setStyleSheet(
            f"font-size:24px; font-weight:700; color:{TX}; border:none;")
        sub = QLabel("Enter patient details to begin recording")
        sub.setStyleSheet(
            f"font-size:12px; color:{T2}; margin-top:4px; border:none;")

        # Cyan underline accent on title
        bar = QFrame()
        ...
        # ── right: dark camera preview ────────────────────────────────────
        right = QWidget()
        right.setObjectName("RP")
        ...
        cap_lbl = QLabel("CAMERA PREVIEW")
        cap_lbl.setStyleSheet(
            f"color:{T2}; font-size:10px; font-weight:700; "
            f"letter-spacing:1.5px; border:none;")
        cap_row.addWidget(self._cam_dot)
    ...
    def mark_camera_ready(self):
        ...
        self.cam_chip.set_text("● Camera ready")
        self.cam_chip.set_kind("success")
...
class LiveScreen(QWidget):
    def __init__(self, main):
        ...
        self.sess_lbl.setStyleSheet(
            f"font-size:14px; font-weight:700; color:{TX}; border:none;")
        ...
        hdr.addWidget(self.sess_lbl)
        hdr.addSpacing(8)
        hdr.addWidget(self.lock_chip)
        hdr.addStretch()
        hdr.addWidget(self.rec_chip)
        hdr.addSpacing(14)
        hdr.addWidget(self.timer_lbl)
        root.addLayout(hdr)
        ...
        def _panel(caption):
            outer = QFrame()
            outer.setObjectName("VP")
            outer.setStyleSheet(
                f"QFrame#VP{{background:#070F18; border-radius:16px; "
                f"border:1px solid {C3};}}")
            fl = QVBoxLayout(outer)
        ...
        self.actual_wrap, self.actual = _panel("Webcam Feed")
        self.pose_wrap,   self.pose   = _panel("Pose Tracking")
        vids.addWidget(self.actual_wrap)
        vids.addWidget(self.pose_wrap)
        root.addLayout(vids)
    ...
    def _tick(self):
        ...
        if self._pulse:
            ...
            self.rec_chip.setStyleSheet(
                ...
                f"border:1px solid #6B2222;")

    def set_locked(self, locked: bool):
        if locked:
            self.lock_chip.set_text("● Patient locked")
            self.lock_chip.set_kind("success")
        else:
            self.lock_chip.set_text("◌  Searching…")
            self.lock_chip.set_kind("caution")
...
class ResultsScreen(QWidget):
    def __init__(self, main):
        ...
        self.title_lbl.setStyleSheet(
            f"font-size:24px; font-weight:700; color:{TX}; "
            f"border:none; background:transparent;")
        self.sub_lbl = QLabel("")
        self.sub_lbl.setStyleSheet(
            f"font-size:13px; color:{T2}; "
            f"border:none; background:transparent; margin-top:2px;")
        v.addWidget(self.title_lbl)
        v.addWidget(self.sub_lbl)
        v.addSpacing(24)

        # ── amber honesty banner ──────────────────────────────────────────
        banner = QFrame()
        banner.setObjectName("BNR")
        banner.setStyleSheet(
            f"QFrame#BNR{{background:#1E1A0A; border-radius:10px; "
            f"border:1px solid #4A3A10; border-left:3px solid {AM};}}")
        brow = QHBoxLayout(banner)
        brow.setContentsMargins(16, 12, 16, 12); brow.setSpacing(10)
        bico = QLabel("⚠")
        bico.setStyleSheet(f"font-size:15px; color:{AM}; border:none; background:transparent;")
        btxt = QLabel(
            "<b>WEBCAM TEST MODE</b> — gait values are approximate "
            "(no depth camera). Use a depth-sensor build for clinical data.")
        btxt.setStyleSheet(
            f"color:#C9A54A; font-size:12px; border:none; background:transparent;")
        ...
        # ── Joint Angles ──────────────────────────────────────────────────
        v.addWidget(SectionHeader("Joint Angles", accent=CY))
        v.addSpacing(12)
        ...
        if HAS_CHARTS:
            ...
            v.addSpacing(12)
            chart_card = QFrame()
            chart_card.setObjectName("CC")
            chart_card.setStyleSheet(
                f"QFrame#CC{{background:{C1}; border-radius:12px; "
                f"border:1px solid {C3};}}")
        ...
        # ── Gait Parameters ───────────────────────────────────────────────
        v.addWidget(SectionHeader("Spatiotemporal Gait Parameters — approximate",
                                   accent=VI))
        v.addSpacing(12)
...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GaitLab Pro — Clinical Edition")
    ...
    def end_session(self):
        ...
        self.results.populate(
            angles, gait, folder, self.records,
            self.patient_id, self.exercise)
    ...
    def angle_summary(self):
        ...
        if np.isfinite(lk).any():
            s["lk_peak"] = float(np.nanmin(lk))
            s["lk_rom"]  = float(np.nanmax(lk) - np.nanmin(lk))
        if np.isfinite(rk).any():
            s["rk_peak"] = float(np.nanmin(rk))
            s["rk_rom"]  = float(np.nanmax(rk) - np.nanmin(rk))
        return s
    ...
    def save_all(self, angles, gait):
        ...
        with open(os.path.join(folder, base+"_info.json"), "w") as f:
            json.dump({"patient_id":self.patient_id,"exercise":self.exercise,
                       ...
                       "frames":len(self.joint_frames),"mode":"webcam_test"}, f, indent=2)
